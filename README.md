...
یا در کنسول لیارا مخزن گیت‌هاب را به برنامه وصل کنید. ساخت حدود ۲ دقیقه طول می‌کشد (خروجی standalone نکست؛ lint و typecheck پیش از push اجرا می‌شوند نه روی سرور ساخت).
...
| خطای 502 بعد از استقرار | برنامه بالا نیامده | لاگ برنامه را ببینید (`liara logs`). `/api/health` کد خطا را می‌گوید: `DATABASE_URL_missing` متغیر نیست؛ `ENOTFOUND` نام میزبان دیتابیس غلط است یا برنامه و دیتابیس در یک شبکهٔ خصوصی نیستند؛ `CONNECT_TIMEOUT` یا `ECONNREFUSED` دیتابیس در دسترس نیست؛ `28P01` نام کاربری یا رمز دیتابیس غلط است |
| ساخت در لیارا بعد از ۵ دقیقه قطع می‌شود | سقف زمان ساخت؛ سرور ساخت گاهی کند است | برای تغییر متغیرها (رمز، دیتابیس و…) ساخت دوباره لازم نیست؛ برنامه خودش راه‌اندازی مجدد می‌شود. فقط برای تغییر کد deploy کنید و اگر قطع شد یک بار دیگر تلاش کنید. اگر `npm ci` کند است `NPM_REGISTRY` را به یک mirror نزدیک بدهید |
