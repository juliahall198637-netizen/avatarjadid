...
| ساخت در لیارا بعد از ۵ دقیقه قطع می‌شود | سقف زمان ساخت | از آخرین نسخه استفاده کنید (`Dockerfile` فقط خروجی standalone را کپی می‌کند)؛ اگر `npm ci` کند است `NPM_REGISTRY` را به یک mirror نزدیک بدهید |
