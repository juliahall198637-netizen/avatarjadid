...
export const GET = route(async (request: Request) => {
  ...
  const rows = await db()`
    select d.id, d.title, d.source, d.chunk_count, d.char_count, d.created_at,
           d.active, count(c.embedding)::int as embedded
...
export const POST = route(async (request: Request) => {
  ...
  if (file instanceof File && file.size) {
    ...
    const result = await addDocument(title || file.name, file.name, text);
    await audit(admin, "افزودن سند", title || file.name);
    return json(result);
  }
  const text = String(form.get("text") ?? "");
  if (!title) throw new ApiError(400, "no_title", "برای متن یک عنوان وارد کنید.");
  const result = await addDocument(title, "متن دستی", text);
  await audit(admin, "افزودن سند", title);
  return json(result);
});
